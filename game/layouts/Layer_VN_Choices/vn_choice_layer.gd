...
func _apply_export_overrides():
	...
	if (!sounds_pressed.is_empty()):
		%DialogicNode_ButtonSound.sound_pressed = load(sounds_pressed)
	if (!sounds_hover.is_empty()):
		%DialogicNode_ButtonSound.sound_pressed = load(sounds_hover)
	if (!sounds_focus.is_empty()):
		%DialogicNode_ButtonSound.sound_pressed = load(sounds_focus)
